...
from pathtokensurv.ablation import ABLATIONS, apply_ablation
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run one frozen-split PathTokenSurv architecture ablation fold.")
    ...
    if primary_manifest.get("status") != "PASS":
        raise RuntimeError("Primary fold manifest must have status PASS before ablation training.")
    ...
    config, definition = apply_ablation(base, args.ablation)
    # Exact model initialization seed used by the corresponding primary outer fold.
    config.training.seed = int(primary_manifest.get("model_seed", base.training.seed + outer_fold * 1009))
    ...
    for name in ["preprocessor.pkl", "pathway_spec.json", "time_discretizer.json", "split_assignments.csv"]:
        path = primary_artifacts / name
        frozen_hashes[name] = sha256_file(path)
    ...
    manifest = {
        ...
        "model_overrides": dict(definition.model_overrides),
        "training_overrides": dict(definition.training_overrides),
        ...
        "reuses_primary_pathway_spec": True,
        "reuses_primary_time_discretizer": True,
        "resume_from": str(args.resume_from) if args.resume_from else None,
        "environment": environment,
    }
    save_json(manifest, output_dir / "ablation_manifest.json")

    print("PathTokenSurv frozen architecture ablation")
    ...
    print("\nHeld-out ablation test metrics")
    for key in ["c_index", "ibs"]:
        if key in result.metrics:
            print(f"  {key}: {result.metrics[key]:.6f}")
    print(f"Ablation fold completed: {output_dir}")
